...
begin
	...
	while @@FETCH_STATUS = 0 begin
		set @TestValue = 0
		if @ScoreCalcType = 2 set @TestValue = @RawScore -- òåñòîâûé ðàâåí ñûðîìó
		else if @ScoreCalcType = 1 begin -- Ò-áàëë
		...
		else if @ScoreCalcType = 3 begin -- ïðîöåíòû
			...
			close c
			deallocate c
			raiserror ('íåâåðíûé òèï ðàñ÷åòà áàëà', 16, 1)
			return
		end
...
go
-- ïåðåîïðåäåëåíèå çíà÷åíèé äëÿ ðàñ÷åòà òåñòîâãî áàëà äëÿ øêàëû
...
begin
	...
	insert into ItemScale_Link (item_id, kf, scale_id, subscale_id) -- õì... à... dimension_id êàêàÿ äîëæíà áûòü ?
...
--	set @res = REPLACE (@res, '[Àâòîð]', @Author)
...
-- ÈËÈ òàêîé âàðèàíò (ïî ïëàíó õóæå, ïî ñìûñëó ... ìîæåò áûòü áîëåå ïðàâèëüíûé)
drop function dbo.SubjectAnswers
go
create function dbo.SubjectAnswers (@SubjectID int) returns @tbl table (answer nvarchar(255)) 
AS
...
--	insert into @tbl (testid, testname)
	...
	(
		(patindex('%ëè÷íîñòü%', @tags)>0 and t.tags like '%ëè÷íîñòü%') or
		(patindex('%ïñèõè÷åñêîå çäîðîâüå%', @tags)>0 and t.tags like '%ïñèõè÷åñêîå çäîðîâüå%') or
		(patindex('%èíòåëëåêò%', @tags)>0 and t.tags like '%èíòåëëåêò%') or
		(patindex('%ðàáîòà%', @tags)>0 and t.tags like '%ðàáîòà%') or
		(patindex('%îòíîøåíèÿ%', @tags)>0 and t.tags like '%îòíîøåíèÿ%') or
		(patindex('%îòíîøåíèÿ%', @tags)=0 and patindex('%ðàáîòà%', @tags)=0 and patindex('%èíòåëëåêò%', @tags)=0 and patindex('%ïñèõè÷åñêîå çäîðîâüå%', @tags)=0 and patindex('%ëè÷íîñòü%', @tags)=0)
	)
	and id not in (
...
begin
	declare @param_id int
	insert into Params (Test_ID, name, Param_Type, Group_ID)
	values (@TestID, 'Âîçðàñò', 2, @GroupID)
	set @param_id = @@IDENTITY
	insert into Param_Values (Param_ID, str_value, ivalue_1, ivalue_2)
...
begin
	...
	select 'Êîïèÿ '+name, author, NEWID(), abbreviature, instruction, 0, Tags, StimulSource, GETDATE(),
...
begin
	...
	if (@delta <= 31)
		set @res= cast(@delta as varchar(10)) +' äí. íàçàä'
	else begin
		set @delta = datediff (mm, @action_date, @for_date)
		if (@delta <= 12)
			set @res= cast(@delta as varchar(10)) +' ìåñ. íàçàä'
		else begin
			set @delta = datediff (yy, @action_date, @for_date)
			set @res= cast(@delta as varchar(10)) +' ã. íàçàä'
		end
	end
	return @res
end
...
returns smallint
with schemabinding -- äëÿ äåòåðìèíèçìà
begin
  if @p_dt is null
    set @p_dt = '19010101'
